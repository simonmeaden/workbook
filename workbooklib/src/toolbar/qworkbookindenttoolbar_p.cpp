...
void QWorkbookIndentToolBarPrivate::init()  {
    pUndentBtn = new QPushButton(q_ptr);
    pUndentBtn->setToolTip("Decrease Indent");
    pUndentBtn->setStyleSheet(q_ptr->buttonStyle);
    pUndentBtn->setContentsMargins(0, 0, 0, 0);
    pUndentBtn->setIcon(QIcon("://undent"));
    q_ptr->addWidget(pUndentBtn);
    q_ptr->connect(pUndentBtn, SIGNAL(clicked()), q_ptr, SLOT(undent()));

    pIndentBtn = new QPushButton(QIcon("://indent"), "", q_ptr);
    pIndentBtn->setToolTip("Increase Indent");
    pIndentBtn->setStyleSheet(q_ptr->buttonStyle);
    pIndentBtn->setContentsMargins(0, 0, 0, 0);
    q_ptr->addWidget(pIndentBtn);
    q_ptr->connect(pIndentBtn, SIGNAL(clicked()), q_ptr, SLOT(indent()));
